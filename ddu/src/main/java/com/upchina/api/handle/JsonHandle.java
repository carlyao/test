...
import com.upchina.util.Exceptions;
...
public class JsonHandle implements Handle {
    ...
    public void to(BusinessResult result) {
        ...
        try {
            ...
            result.setResultCode(FORMAT_ERROR_CODE);
            result.setResultMsg(FORMAT_ERROR_MSG);
            return;
        }
        ...
        if (!success) {
            if (errCodePath == null) {
                result.setResultCode(textValue);
            }
            if (errMsgPath != null) {
                result.setResultMsg(textValue(rootNode, errMsgPath));
            } else if (errMsgMap != null) {
                result.setResultMsg(errMsgMap.get(textValue));
            }
        } else {
            if (orderIdPath == null) {
