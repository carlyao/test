...
public class JedisUtil {
  ...
  private void init(){
    Properties properties= PropertyUtils.getPropertiesByPath("/properties/application.properties");
  ...
  public JedisUtil(int database) {
    if (pool == null||pool.getResource().getDB().longValue()!=database) {
      init();
      JedisPoolConfig config = new JedisPoolConfig();
      // 控制一个pool可分配多少个jedis实例，通过pool.getResource()来获取；
      // 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
...
//    jedisUtil.setex("test","testvalue",30);
//    System.out.println(jedisUtil.get("test"));
    JedisUtil jedisUtil2=new JedisUtil(6);
    jedisUtil2.setex("testdb6","testvalue",30);
  }
