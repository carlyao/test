...
@Service("portfolioRankHisService")
public class PortfolioRankHisService  extends BaseService<PortfolioRankHis, Integer>{
    ...
    public boolean isExist(String inputName,String value,int id) {
        Example exp = new Example(PortfolioRankHis.class);
        Example.Criteria criteria = exp.createCriteria();
        criteria.andEqualTo(inputName, value);
        List<PortfolioRankHis> list=selectByExample(exp);
	...
	public int getCountByDate(String date) {
		int count = portfolioRankHisMapper.getCountByDate(date);
		return count;
	}
}